...
function sendData( data ) {
    ...
    // Set up our request
    XHR.open( 'POST', 'http://localhost:3000/post_register_form' );
...
function registrar() {
    const gender = document.querySelector('input[name="gender_radio"]:checked')
    if (cpfValid && nameValid && surnameValid && emailValid && passwordValid && gender) {
        var registerForm = {
            "cpf": cpf.value,
